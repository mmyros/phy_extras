...
class ClusterMetricsPlugin(IPlugin):
    def attach_to_controller(self, controller):
        ...
        def amplitude_cutoff(cluster_id):
            ...
            # spike_ids = controller._get_amplitude_spike_ids(cluster_id)
            amplitudes = controller.get_spike_raw_amplitudes(spike_ids)
            return spike_io.QualityMetrics.amplitude_cutoff(amplitudes)

        controller.cluster_metrics['isi_viol'] = controller.context.memcache(isi_viol)
        controller.cluster_metrics['pres_rat'] = controller.context.memcache(presence_ratio)
        controller.cluster_metrics['amp_cut'] = controller.context.memcache(amplitude_cutoff)
        # =============================================================================================
        ...
        #     isolation_distance, l_ratio = spike_io.QualityMetrics.mahalanobis_metrics(
        #         pcs_for_this_unit=pc_features[spike_clusters == cluster_id, :],
